...
public:
    ...
    BIMANDER_M_IS::BIMANDER_M_IS bimander_m_is = BIMANDER_M_IS::N_HALF; // BIMANDER
    int bimander_m = 3; // BIMANDER
    int k_product_minimum_lit_count_for_splitting = 10; // KPRODUCT
    int k_product_k = 2;    // KPRODUCT
    int commander_encoding_k = 3; // COMMANDER
    int64_t MAX_CLAUSES_PER_CONSTRAINT = 1000000; // PB
    bool use_formula_cache = false;
    bool print_used_encodings = false;
    bool check_for_dup_literals = false;
    bool use_gac_binary_merge = false; // BINARY_MERGE
    bool binary_merge_no_support_for_single_bits = true; // BINARY_MERGE
    bool use_recursive_bdd_test = false;
    bool use_real_robdds = true;
    bool use_watch_dog_encoding_in_binary_merger = false; // BINARY
